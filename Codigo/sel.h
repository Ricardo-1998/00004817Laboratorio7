...
Matrix createLocalK(int e,mesh &m){
    ...
    Matrix K,g_matrix,g_matrix_t,Alpha,Beta,Alphat,Betat,BP,BPt;
    ...
    productRealMatrix((J*lambda)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BP,2,2,3),6,2,3),matrixH);

    //Preparando matrixL
    delta = m.getParameter(DELTA);
    productRealMatrix((-1)*(delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BP,2,2,3),2,2,3),6,2,3),matrixL);

    //Preparando matrixD
    productRealMatrix(J/(6*D),productMatrixMatrix(BPt,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixD);
